// device.cpp - Controller detection and polling (wired + WaveBird)
// Originally from pico-crossing, reworked with Joybus device flag detection
...
bool detect_and_init_device(DeviceState* device) {
	...
	if (absolute_time_diff_us(device->next_retry_time, now) > 0) {
		...
		printf("[Device] Probe response: %02x %02x %02x (ID=0x%04x)\n",
		       response[0], response[1], response[2], device_id);

		if (device_id == DEVICE_ID_KEYBOARD) {
			device->is_keyboard = true;
			device->device_id = device_id;
			device->initialized = true;
			printf("[Device] Keyboard detected (0x%04x)\n", device_id);
			return true;
		} else if (device_id & JOYBUS_FLAG_DEVICE) {
			// Accept any device with the GC device flag (bit 11) set:
			// 0x0900 = wired controller (DEVICE | STANDARD)
			// 0xA800 = WaveBird receiver (DEVICE | WIRELESS | NO_MOTOR)
			// 0xE800 = WaveBird receiving  (+ WIRELESS_RECEIVED)
			device->is_keyboard = false;
			device->device_id = device_id;
			device->initialized = true;
			bool wireless = (device_id & JOYBUS_FLAG_WIRELESS) != 0;
			printf("[Device] Controller detected (0x%04x)%s\n", device_id,
			       wireless ? " [wireless]" : "");
			return true;
		}

		printf("[Device] Unknown device ID: 0x%04x, retrying...\n", device_id);
		device->next_retry_time = make_timeout_time_ms(RETRY_DELAY_MS);
		return false;
	}
